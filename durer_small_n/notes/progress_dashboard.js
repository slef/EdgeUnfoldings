...
function caseProgress(id,compact=false) {
  ...
  if (id==='t_octa') {
    ...
    body+=`<p><b>New conjecture: try just ${d.pattern.candidate_trees} specified trees.</b> They survived a separate ${d.pattern.holdout_samples.toLocaleString()}-shape numerical holdout. The geometric proof has ${d.pattern.proof_classes} symmetry classes of possible simultaneous failure, with ${d.pattern.proved_classes} whole classes excluded so far. <a href="#minus_pair">See the algorithm, figure, and proof target</a>.</p>`;
    ...
    body+=`<p class="progress-change"><b>Latest advance: nine trees → one fixed tree; 409 cells → 16.</b> The certified extent remains ±0.05. The two-tree rule outside this box is still conjectural.</p>`;
  } else if (id==='t_prismd') {
...
function progressComparison() {
  ...
  return `<details class="progress-domain progress-comparison"><summary>Retained before-and-after snapshots</summary><div class="progress-table-wrap"><table class="progress-table"><thead><tr><th>Measure</th>${rows.map(r=>`<th>${r.label}</th>`).join('')}</tr></thead><tbody><tr><td>Fully proved six-vertex types</td>${rows.map(r=>`<td>${r.proved_types} / 7</td>`).join('')}</tr><tr><td>Octahedron: angle branches proved</td>${rows.map(r=>`<td>${r.octahedron_angle_branches} / 3</td>`).join('')}</tr><tr><td>Octahedron: local pair obligations proved</td>${rows.map(r=>`<td>${r.octahedron_local_pairs} / 3</td>`).join('')}</tr><tr><td>Minus-edge: certified half-width</td>${rows.map(r=>`<td>±${progressNumber(r.minus_half_width)}</td>`).join('')}</tr><tr><td>Minus-edge: trees used on the same box</td>${rows.map(r=>`<td>${r.minus_trees}</td>`).join('')}</tr><tr><td>Minus-edge: certificate cells</td>${rows.map(r=>`<td>${r.minus_cells}</td>`).join('')}</tr><tr><td>Prism: certified fraction of box A</td>${rows.map(r=>`<td>${coveragePercent(r.prism_box_A_fraction)}</td>`).join('')}</tr></tbody></table></div><p>The earlier hour added exact counterexamples and a metric-closure obstruction. The later pattern investigation reduced the minus-edge box certificate from nine trees to one and proposed a two-tree universal rule. No additional whole type has been proved. <a href="../../n6/CONTINUATION.md">Retained research log</a>.</p></details>`;
}
