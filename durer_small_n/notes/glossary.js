...
const GLOSS = {
  ...
  D: { label: 'Wedge D', re: /\bwedge D\b|\bD-lemma\b/g, link: '#F_D', short: 'For a triple with middle petal <i>V<sub>j</sub></i>: the intersection of the two closed half-planes bounded by the cut-edge lines through <i>u<sub>j</sub>v<sub>i</sub></i> and <i>u<sub>j+1</sub>v<sub>j+1</sub></i> on the sides away from <i>V<sub>j</sub></i>. When both rotated base angles lie between 0 and π, every common point of the outer petals lies in D. The unrestricted assertion is false.' },
  ...
  triple: { label: 'Triple Lemma', re: /\bTriple Lemma\b/g, all: true, link: '#F_triple', short: 'For three consecutive fan faces <i>W<sub>i</sub></i>, <i>W<sub>j</sub></i>, <i>W<sub>j+1</sub></i> (in any net where they are contiguous) with petals <i>V<sub>i</sub></i>, <i>V<sub>j</sub></i>, <i>V<sub>j+1</sub></i>: under (H) the outer petals <i>V<sub>i</sub></i> and <i>V<sub>j+1</sub></i> are disjoint. Case A and the large fan-angle branch are proved; the fan-angle sum below π remains open.' },
