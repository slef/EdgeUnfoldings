...
const GLOSS = {
  ...
  Zk: { label: 'The net Z<sub>k</sub>', re: /\bZ<sub>k<\/sub>|\bnets? Z/g, link: '#defs-octa', short: 'The unfolding whose cut tree is the star of <i>v</i> (its four edges) plus the single edge <i>w u<sub>k</sub></i>. Hinges: the four equator edges and the other three edges at <i>w</i>.' },
  slit: { label: 'Slit', re: /\bslit\b/g, link: '#defs-octa', short: 'The cut edge <i>w u<sub>k</sub></i> at the antipode; the fan opens there and <i>u<sub>k</sub></i> gets two copies, <i>u<sub>k</sub></i> and <i>u<sub>k</sub></i>′.' },
  ...
  nearness: { label: 'Far and local pairs', re: /\bfar pairs?\b|\blocal pairs?\b/g, link: '#t_octa', short: 'Of the 28 face pairs of <i>Z<sub>k</sub></i>, nine share no net vertex: three local pairs at the slit (across the gap at <i>w</i>) and six far pairs (the two opposite-petal pairs and the four petal-versus-far-fan pairs).' },
};
...
function linkTerms(root) {
  ...
  const walker = document.createTreeWalker(root, NodeFilter.SHOW_TEXT, { acceptNode(n) { let p = n.parentNode; while (p && p !== root) { if (SKIP.has(p.tagName) || (p.classList && (p.classList.contains('term') || p.classList.contains('chip') || p.classList.contains('crumbs') || p.classList.contains('statement-title')))) return NodeFilter.FILTER_REJECT; p = p.parentNode; } return NodeFilter.FILTER_ACCEPT; } });
  const nodes = []; let n; while ((n = walker.nextNode())) nodes.push(n);
  for (const node of nodes) {
    const text = node.nodeValue; const hits = [];
    for (const key of keys) { const g = GLOSS[key]; g.re.lastIndex = 0; let m; while ((m = g.re.exec(text))) { if (!g.all && (counts[key] || 0) >= 1) break; hits.push([m.index, m.index + m[0].length, key]); counts[key] = (counts[key] || 0) + 1; if (!g.all) break; } }
