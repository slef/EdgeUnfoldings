...
## A possible further shortening remains separate

Omitting the comparison of opposite-pair differences is still an open
shortcut. An exact metric example passes that unranked choice but violates
the upper cap bound, even though its net succeeds. The evidence and missing
proof step are retained in [OCTA_UNRANKED_SHORTCUT.md](OCTA_UNRANKED_SHORTCUT.md).
The proved rule above keeps the comparison.
