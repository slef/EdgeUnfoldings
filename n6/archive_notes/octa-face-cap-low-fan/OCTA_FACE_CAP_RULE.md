...
Discard a proposal if its fan curvature exceeds pi. Among the remaining
proposals, choose one whose U is at most 3*pi. Such a proposal always exists:

* If a,b<=pi, then

      U_1+U_2=4*pi+M-m<6*pi,

  since M<2*pi and m>0. The two U's cannot both exceed 3*pi; indeed
  at least one is strictly below it.
* If a>pi, then A>=a>pi, and positivity with total curvature 4*pi forces
  b<pi. Moreover

      U_2=4*pi-(m+a+B)<3*pi.

  Thus the second proposal works. The argument for b>pi is symmetric.
  Both a and b cannot exceed pi, since A+a+B+b would already exceed 4*pi.

For the chosen proposal, both blue-face sums are at most its U, so their
upper bounds in (C) hold. If M>=pi, both sums are strictly greater than pi.
If M<=pi, maximality makes all six curvatures at most pi; the three
complementary vertices sum to at most 3*pi, giving each lower bound.
...
curvature makes the local slit threshold automatic. Only two fan choices
need to be compared, and the two face-cap sums coordinate the finite-edge
and opposite-petal arguments at the same opening.

For this existence proof, the geometric core needs the low-fan half-span
lemma, local separator, finite-entry containment and sine inequality,
